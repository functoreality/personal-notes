* Truman AI 操盘手案例拆解，确认构建所用要素可从当前笔记框架推出；{_q9g96l}
	...
	* skill 编写；((q9de6x))
		...
		* 可用参考内容素材
			* 本框架组织：按“主要内容角色 → 来源 × 形态”展开，具体材料直接写在第二层
				* 作用：先看出准备重心，再看它靠哪种来源、哪种素材支持
			* 阅读边界：不按文档数量统计。项目文档、DataPack、口述初稿的粒度不同，只能做定性分布判断
				* `16b.webp` 只证明项目文档与最终提示词的规模，不能用来统计各类材料的数量
				* `14i.webp` 中的“场景”“基本功”“创造力”不直接归入下方素材分布
					* 场景是任务边界，基本功是工具与执行能力，创造力是探索方向。它们决定寻找什么材料和如何使用，但本身不是一项参考材料
				* 素材类型的边界：“案例、方法、效果”能区分大部分参考材料，但一堂与 Truman 系统介绍、品牌理念、故事等对象背景不宜硬归入其中
					* 这里暂记为“对象、背景事实”，不立即推出必须在 `useAI.md` 增加第四种封闭类型
					* 若“案例”本意是包含当前对象的具体情况，则只需在原笔记说明这个口径，而非新增类型
					* Truman 美好想象是“目标、期望效果”，不同于对方法实测效果的证据；这里暂与“对象、背景事实”分开记录
						* 若“效果”打算同时包括目标效果与实测效果，需在原笔记显式区分两者
			* 总体分布
				* 角色重心：“素材”与“执行策略方法”最厚，“审美”材料不多但决定方向；启动、终止、输出形态、目标核查中，只有输出与核查被较多地显式设计
				* 来源重心：半内源材料是主体，包括品牌知识、历史口播、选题、方法论与 KnowHow；外源主要用于对标；内源主要用于统筹、建模、封装和测试
				* 形态重心：方法与案例都很密集，实测效果证据出现得较晚，集中在测试环节；对象、背景事实则是 DataPack 的基础
			* 主要内容角色—对象、背景事实：（半内源 × ？）一堂与 Truman 系统介绍、品牌定位、反常识认识、稀缺性、边界、一堂故事和理念
			* 主要内容角色—任务素材
				* 半内源 × 案例：历史口播 DataPack 与逐字稿，它们是一堂既往内容产出的具体样本
				* 半内源 × 方法：我的 Top 话题清单、Top 网感选题
					* 这两类清单的最初来源未展示，因而只能判断为“已被该项目内化的素材”
				* 外源 × 案例：易立竞访谈参考与 Top 访谈专家的调研，提供可观察的访谈样本
			* 主要内容角色—审美（验收标准）
				* 外源 × 案例：调研 Top 访谈专家，作为什么是“好访谈”的参照系
				* 内源 × 方法：Truman 美好想象，对期望内容效果的当场外显，不是已得到的测试结果
				* 内源 × 方法：雷达图建模，将上述审美转成可复核的维度
				* 内源 × 方法：8 个评分维度、7 个一票否决项
				* 半内源 × 方法：过往 KnowHow 作为对网感、深度和品牌一致性的既有判断依据
			* 主要内容角色—执行策略方法（执行过程）
				* 半内源 × 方法：讲香方法论 DataPack、十指讲香、ABACC、SABC 选题、干货六级、四层问题链、时长结构和标题模板
				* 内源 × 方法：将领域方法组装为基础工作流、角色、S0 至 S8 状态机、4 种输出模式、顶层文档和按功能拆分的项目文档
					* `14j.webp`、`14l.webp`、`16a.webp`、`16c.webp` 表明，内源工作的重点是结构化、封装和调度，不是用它们取代领域方法本身
			* 次要内容角色—目标核查
				* 内源 × 方法：10 个上线前黄金测试题、自动化测试方案与 A、B 两版封装对比
				* 内源 × 效果：测试执行记录、测试结论和调试 2 至 3 轮的结果
					* 材料展示了记录和结论文档的存在，但未展示具体分数、AB 差异或修改归因
			* 次要内容角色—输出形态
				* 内源 × 方法：访谈、构思、成稿、返修 4 种输出模式，使访谈不只止于现场问答
			* 次要内容角色—启动与终止
				* 未见专门准备材料：案例展示的是人工开启访谈或内容生产，但没有给出触发规则、终止阈值或状态转移条件
			* 对当前框架的启示
				* 这是“半内源坚实地基 + 外源对标定标高 + 内源统筹封装与测试”的组合，不是只靠外部调研找一个现成方法
				* 其中“效果”材料并非构建前的主要输入，而是在初版封装后通过测试形成，主要服务于后续迭代
				* 启动与终止未被材料准备阶段明确覆盖。这不能证明它们不重要，只说明当前产品是人工启动的、以访谈流程为主的 MVP
		* 首次起草
