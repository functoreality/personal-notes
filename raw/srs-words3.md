x}# 英语单词 part 3
	* 2025-09-05 创建
* 单词读音（动词、名词）：compound{p95k34}
	* 名词读作 /'kɒmpaʊnd/，动词读作 /kəm'paʊnd/
	* 注意重音、o1
* 单词含义（名词，2+1 种）：compound{p95k36}
	* Organic compounds contain carbon in their [molecules].
		* (化学中的)化合物
	* [Honey] is basically a compound of water, two types of sugar, vitamins and enzymes.
		* 混合物;组合物
	* Police fired on them as they fled into the [embassy] compound.
		* (作特殊用途的)大院，有围墙的场地
* 单词含义（动词，2+1 种）：compound{p95k38}
	* The DNA molecule is compounded from many smaller molecules.
		* (formal)由…构成（或形成） to be formed from sth
	* liquid soaps compounded with [disinfectant]
		* [often passive] (formal ortechnical 术语)混合；掺和；拌和 to mix sth together
	* The [problems] were compounded by severe food shortages.
		* 使加重；使恶化
* 单词读音：cohort{p9ib31}
	* /ˈkəʊhɔːt/
	* 注意重音
* 单词含义（两种）：cohort{p9ib33}
	* the 1999 [birth] cohort (= all those born in 1999)
		* 有共同特点或举止类同的）一群人，一批人
	* Robinson and [his] cohorts were soon ejected from the hall.
		* (disapproving)同伙；支持者
* 单词含义（名词，原义+比喻）：hedge{p9lk88}
	* 树篱 a row of bushes or small trees planted close together, usually along the edge of a field, garden/yard or road
	* to buy gold as a hedge against [inflation]
		* ~ against sth 防止损失（尤指金钱）的手段
* 单词含义（动词，两种）：hedge{p9lk8a}
	* Just [answer] ‘yes’ or ‘no’─and stop hedging.
		* 避免正面回答；不直接许诺；拐弯抹角
	* Their [offer] was hedged around with all sorts of conditions.
		* (formal)包围；限制
* 单词含义（原义+比喻）：ripple{p9rh6n}
	* The air was so still that there was hardly a ripple on the [pond]'s surface.
		* You throw a pebble in a [pool] and it ripples...
		* 波纹；细浪；涟漪；（使）如波浪般起伏
	* The announcement sent a ripple of [excitement] through the crowd.
		* A [chill shiver] rippled over his skin.
		* 逐渐扩散的感觉；感觉等扩散；涌起
	* 注：名词动词都是
* 单词含义（对事+对人）：spoil{p9rh7q}
	* Our camping trip was spoilt by bad weather.
		* The tall buildings have spoiled the view.
		* 破坏；搞坏；糟蹋；毁掉；同义词：ruin
	* She spoils those kids of hers.
		* Why not spoil yourself with a weekend in a top hotel?
		* [动词+名词短语] 溺爱；娇惯；宠坏（同义词：overindulge）善待；格外关照
* 单词读音：dispose{p9u97j}
	* /dɪˈspəʊz/
	* 注意 s2
* 短语含义（对事、物、人）：dispose of{p9u97l}
	* You did us a great favour by disposing of that [problem].
		* 应付；解决；处理
	* the difficulties of disposing of nuclear [waste]
		* 去掉；清除；销毁
	* It took her a mere 20 minutes to dispose of her [opponent].
		* 击败；杀死
* 单词含义：lipid{pa2l25}
	* 脂质
* 单词读音：solicit{pa3l2h}
	* /səˈlɪsɪt/
	* 注意重音，o
* 单词含义：solicit{pa3l2j}
	* They were planning to solicit [funds] from a number of organizations.
	* [Historians] and [critics] are solicited for their opinions.
	* (formal)索求，请求…给予（援助、钱或信息）；征求；筹集
* 单词读音（英美）：synopsis{pa8c19}
	* 英/sɪˈnɒpsɪs/ 美/sɪˈnɑːpsɪs/
	* 注意重音，o
* {old}# 单词含义：synopsis
	* For each title there is a brief synopsis of the book.
	* (著作或作品的)大纲，提要，概要，梗概
* 单词含义比较：synopsis，syllabus{pa8c1b}
	* For each title there is a brief synopsis of the [book].
		* (著作或作品的)大纲，提要，概要，梗概
	* 教学大纲 a list of the topics, books, etc. that students should study in a particular subject at school or college
* 单词读音（英美）：arduous{padg4k}
	* 英/ˈɑːdjuəs/ 美/ˈɑːrdʒuəs/
	* 注意重音，d
* 单词含义：arduous{padg4m}
	* a long, hot and arduous [journey]
	* 困难的；艰巨的；费力的
* 单词读音：anatomy{pak86b}
	* /əˈnætəmi/
	* 注意重音，a1,a2,o
* 单词含义：anatomy（1 原始 + 2 引申）{pak86d}
	* the anatomy of the [horse]
		* （动植物的）结构，解剖；解剖学
	* Various parts of [his] anatomy were clearly visible.
		* (humorous)人体 a person's body
	* an anatomy of the current [recession]
		* (formal)剖析；解析 an examination of what sth is like or why it happens
	* 注：词根可能是 atom
* 单词读音：paramount{pbcg7d}
	* /ˈpærəmaʊnt/
	* 注意重音，a2
* 单词含义（两种）：paramount{pbcg7f}
	* Safety is paramount.
		* 至为重要的；首要的
	* China's paramount leader
		* (formal)至高无上的；至尊的；权力最大的
* 单词读音（英美）：futile{pbhb8u}
	* 英/ˈfjuːtaɪl/ 美/ˈfjuːtl/
	* 注意重音、u、i
* 单词含义：futile{pbhb8w}
	* Their [efforts] to revive him were futile.
	* It would be futile to protest.
	* 徒然的；徒劳的；无效的；同义词：pointless
* 单词读音：impinge{pbke21}
	* /ɪmˈpɪndʒ/
* 单词含义：impinge{pbke23}
	* He never allowed his work to impinge on his private [life].
	* the cuts in defence spending that have impinged on two of the region's largest [employers]
	* (formal)对…有明显作用（或影响）；妨碍；侵犯 to have a noticeable effect on sth/sb, especially a bad one 同义词：encroach
* 单词读音：succinct{pbke2r}
	* /səkˈsɪŋkt/
* 单词含义（同义词）：succinct{pbke2t}
	* Keep your [answers] as succinct as possible.
	* (approving)简明的；言简意赅的；同义词：concise
* 单词读音：contingent{pbna33}
	* /kənˈtɪndʒənt/
	* 注意重音，g
* 单词含义（名词，两种）：contingent{pbna35}
	* A strong contingent of local [residents] were there to block the proposal.
		* （志趣相投、尤指来自同一地方的）一组与会者，代表团
	* the French contingent in the UN peacekeeping [force]
		* （军队的）分遣队，小分队
* 单词含义（形容词）：contingent{pbna37}
	* All payments are contingent upon satisfactory completion [dates].
	* In effect, growth is contingent on improved [incomes] for the mass of the low-income population.
	* ~ (on/upon sth) (formal)依情况而定的 depending on sth that may or may not happen
* 单词读音：arsenal{pc1e1f}
	* /ˈɑːsənl/
	* 注意重音，e
* 单词含义（原义-引申-比喻）：arsenal{pc1e1h}
	* Britain's nuclear arsenal
		* （统称）武器 a collection of weapons such as guns and explosives
	* The murder weapon was part of a haul stolen in a raid on a South African air-force arsenal in 1990.
		* 武器库；军火库
	* Managers use a full arsenal of motivational techniques to get employees to take risks.
		* （方法、资源等的）宝库；(储藏工具的)仓库
* 单词读音：niche{pc3g7g}
	* /niːʃ/
* 单词含义（原义+引申人+引申物）：niche{pc3g7i}
	* Above him, in a niche [on the wall], sat a tiny veiled Ganesh, the elephant god.
		* 壁龛;(山体或悬崖上的)凹洞
	* He eventually found his niche in sports [journalism].
		* 称心的工作;合适的活动
	* I think we have found a niche in the toy [market].
		* (有自己特定的需求、目标顾客和产品的)小众市场，缝隙市场，利基市场;商机
* 单词含义（原义2+引申1）：flicker{pc7j75}
	* Her [eyelids] flickered as she slept
		* 快速摆动；颤动；抖动；拍动
	* the flickering screen of the television
		* The [lights] flickered and went out.
		* 灯光或火焰闪烁；闪现；忽隐忽现；摇曳
	* [Anger] flickered in his eyes.
		* 情绪、思想等闪现；一闪而过
* 单词读音：endeavor{pc8c0s}
	* /ɪnˈdɛvə/
	* 注意重音，e1,ea
* 单词含义：endeavor{pc8c0u}
	* Please make every endeavour to arrive on time.
		* n.（尤指新的或艰苦的）努力，尝试
	* I will endeavour to do my best for my country.
		* v.(formal)努力；尽力；竭力；同义词：strive
* 单词读音：decimate, decimation{pc8d7v}
	* /ˈdesɪmeɪt/，/ˌdɛsɪˈmeɪʃən/
	* 注意重音，e1
* 单词含义：decimate（具体、抽象）, decimation{pc8d7x}
	* The [rabbit] population was decimated by the disease.
		* 大量毁灭，大批杀死（某地区的动物、植物或人）
	* Cheap imports decimated the British cycle [industry].
		* (informal)严重破坏；大大削弱
	* 十中抽一，抽取
* 单词读音：culminate{pc9a36}
	* /ˈkʌlmɪneɪt/
	* 注意重音，u
* 单词含义：culminate{pc9a38}
	* Months of hard work culminated in [success].
	* a gun battle which culminated in the [death] of two police officers
	* (formal)（以某种结果）告终；（在某一点）结束
* 单词读音（英美）：posit{pc9a3k}
	* 英/ˈpɒzɪt/ 美/ˈpɑːzɪt/
* 单词含义：posit{pc9a3m}
	* Most religions posit the existence of life after death.
	* (formal)假设；认定；认为…为实；同义词：postulate
* 单词读音（英美）：contemplate{pcal61}
	* 英/ˈkɒntəmpleɪt/ 美/ˈkɑːntəmpleɪt/
	* 注意重音，o,e1
* 单词含义（1+2）：contemplate{pcal63}
	* He contemplated his [hands], still frowning.
		* 凝视;注视；同义词：stare at
	* For a time he contemplated a [career] as an army medical doctor.
		* 考虑;思量;思忖
	* As he lay in his hospital bed that night, he cried as he contemplated his [future].
		* It is a place of quiet contemplation.
		* 深思熟虑;沉思;苦思冥想
* 单词读音：transcend{pcdg11}
	* /trænˈsend/
* 单词含义：transcend{pcdg13}
	* ...issues like European union that transcend party loyalty.
	* 超越;超出；同义词：exceed
* 单词含义（名词，3 种联句）：graft{pcmb6d}
	* The graft of planting trees was hard graft, unlike the quick gains from political graft.
	* （植物）嫁接/（器官）移植，艰苦的工作，贿赂
* {x}# 所用 prompt
	* graft 作为名词有多种含义（嫁接/移植，重活，贿赂）。请给我造一个英文句子，其中同时用到这几种含义。我希望将这个句子用于教学，因此希望 1. 这个句子能简单一些，2. 几种含义的出现顺序无关紧要，3. 句子应该描述一个完整的画面或者场景，不应该是多个独立场景的简单并列。
	* 提示词要点：1. 列出含义列表；如果只说“三种含义”，会取“重活”之外的 3 种；2. 要开 CoT；3. 可多试几次。
	* v1 不够好：In the small orchard, the graft of a new shoot onto the old tree was backbreaking graft, but the honest farmer never turned to graft to ease his work.
	* v2a “The young official, tired of the daily graft in the city parks, finally learned that true growth comes not from political graft, but from the careful graft of a single tree.”
	* v2b “For the gardener, the graft of planting trees was hard graft, unlike the quick gains from graft in town.”
* 单词含义（动词，2原义+1引申）：graft{pcmb6f}
	* The top layer of [skin] has to be grafted onto the burns.
		* 移植
	* Pear [trees] are grafted on quince rootstocks.
		* 嫁接
	* The Japanese tried to graft their own [methods] on to this different structure.
		* 将…接入（到另一思想或制度）
		* Old [values] are being grafted onto a new social class.
		* 使（思想、制度等）成为（…的一部分）；植根 to make one idea, system, etc. become part of another one
* 单词读音：rescind{pcmc09}
	* /rɪˈsɪnd/
	* 注意重音，i2
* 单词含义：rescind{pcmc0b}
	* Trade Union leaders have demanded the government rescind the [price rise].
	* 废除;废止;撤销;停止；同义词：revoke
* 单词读音：imperative{pcpd8j}
	* /ɪmˈperətɪv/
	* 注意重音
* 单词含义（1原义+2引申）：imperative{pcpd8l}
	* an imperative [sentence]
		* (grammar 语法)表示命令的；祈使的
	* It is imperative to continue the [treatment] for at least two months.
		* (formal)重要紧急的；迫切的；急需处理的
	* an imperative [tone]
		* (formal)表示权威的 expressing authority
* 单词读音：inundate{q14b06}
	* /ˈɪnʌndeɪt/
* 单词同义词（原义+引申）：inundate{q14b08}
	* Their neighborhood is being inundated by the rising [waters] of the Colorado River.
		* 淹没;泛滥；同义词：flood
	* Her office was inundated with [requests] for tickets.
		* (信件、要求、请求等)使应接不暇,使不胜负荷；同义词：overwhelm
* 单词读音（英美）：apocalypse{q2hc02}
	* 英/əˈpɒkəlɪps/ 美/əˈpɑːkəlɪps/
* 单词含义：apocalypse{q2hc04}
	* We live in the shadow of the apocalypse, of a catastrophe that will mean the end of the world itself.
	* 世界覆灭；世界末日
* 单词读音：perpetual{q2ka7x}
	* /pəˈpetʃuəl/
* 单词含义：perpetual{q2ka7z}
	* the perpetual [noise] of traffic
	* He was elected perpetual [president].
	* 不间断的；持续的；长久的；同义词：continuous
* 单词读音：trough{q33h01}
	* /trɒf/
* 单词含义：trough{q33h03}
	* The boat rolled heavily in the troughs between the [waves].
		* (两个大浪之间的)波谷
	* The old stone [cattle] trough still sits by the main entrance.
		* (喂家畜的)饲料槽，饮水槽
* 单词含义：delegate{q36d7s}
	* [Officials] have now been delegated to start work on a draft settlement.
		* He plans to delegate more [authority] to his deputies...
		* （动词，对事or人）授权，委派（做）
	* The conference was attended by delegates from 56 countries.
		* （名词）代表；会议代表
* 单词读音：eligible{q36h1x}
	* /ˈelɪdʒəbl/
* 单词含义（两种）：eligible{q36h1z}
	* Almost half the population are eligible to [vote] in today's election.
		* 有资格的;符合条件的;有能力的
	* He's the most eligible [bachelor] in Japan.
		* （作为结婚对象）合适的，中意的
* 单词读音：dossier{q36k7g}
	* /ˈdɒsieɪ/
	* 注意 er
* 单词含义：dossier{q36k7i}
	* The company is compiling a dossier of [evidence] to back its allegations...
	* 档案;卷宗;资料汇编
* 单词读音（动词、形容词）：intimate{q36k81}
	* 形容词读作 /'ɪntɪmət/,动词读作 /'ɪntɪmeɪt/
* 单词含义（动词）：intimate{q36k83}
	* He has already intimated to us his [intention] to retire.
	* (formal)透露；（间接）表示；暗示；同义词：make known
* 单词含义（形容词，3 种）：intimate{q36k85}
	* 总结：人之间，人对事，事之间（类似((p71g1n))reconcile）
	* intimate [friends]
		* 人亲密的；密切的
	* an intimate [knowledge] of the English countryside
		* 知识详尽的；精通的 very detailed and thorough
	* an intimate [connection] between class and educational success
		* 事物间的联系密切的；紧密的 very close
* 单词含义（4 种搭配）：stale{q54b4i}
	* 总结：stale food/air/idea, feel stale
		* （可自行想象联合场景）
	* (食物)不新鲜的，变味的
	* (空气、气味)不新鲜的，污浊的
	* Their [marriage] had gone stale .
		* (地方、活动或观点)令人厌倦的，缺乏新鲜感的
	* After ten years in the job, she [felt] stale and needed a change.
		* 厌倦的;腻烦的
* 单词含义（同义词）：dormant{q5sa4b}
	* a dormant [volcano]
	* During the winter the [seeds] lie dormant in the soil.
	* 休眠的；蛰伏的；暂停活动的；同义词：inactive
	* （评）词根似乎是 dorm，因此含义不难理解
* 单词含义（同义词）：bogus{q7il5g}
	* a bogus doctor/contract
	* bogus claims of injury by workers
	* 假的；伪造的；同义词：false
* maintain 名词形式{q8qn5b}
	* maintenance
	* 注意不是 ai 而是 e
